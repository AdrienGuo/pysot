...
class Augmentation:
    ...
    def __call__(self, image, bbox, ratio=None, bg=None, context_amount=None):
        """
        Returns:
            image: 圖片
            boxes: gt boxes
        ...
        if self.type == "template":
            template_image, template_ratio, box, origin_box = self._template_crop(image, bbox, bg, context_amount)
            # box, origin_box: 只有在 testing 畫圖的時候會用
            return template_image, template_ratio, box, origin_box

        if self.type == "search":
            search_image, boxes, r, spatium = self._search_crop(image, bbox, ratio)
